...
*/
	...
	if (pHalData->BoardType == BOARD_USB_COMBO)
	{
		if (pHalData->SlimComboDbg == _TRUE)
		{
			...
			// 1. SIC?Test Mode \A4\A4, Debug Ports \B7|\A6۰\CA Enable, \A9ҥH Driver \A4W\A8ӫ\E1, 
			//	\ADn\C3\F6\B1\BC\BDг]\A9w 0x 00[7] -> "1", \B1N\A5\A6 Disable.   effect if not: power consumption increase
			rtw_write8(padapter, REG_SYS_ISO_CTRL, rtw_read8(padapter, REG_SYS_ISO_CTRL)|BIT7);

			// 2. SIC?Test Mode \A4\A4, GPIO-8?\B7| report Power State \A9ҥH Driver \A4W\A8ӫ\E1, \BDг]\A9w? 0x04[6] -> "1" \B1N\A5\A6 Disable
			...
			// 3. SIC Test Mode \A4\A4, EESK, EECS \B7| report?Host Clock status, \A9ҥH Driver \A4W\A8ӫ\E1, \BDг]\A9w? 0x40[4] -> "1" \B1N\A5\A6\A4\C1\A6\A8 EEPROM \A8ϥ\CE Pin (autoload still from Efuse)
			...
			// 4. SIC Test Mode \A4\A4,?SIC Debug ports \B7|\A6۰\CA Enable , \A9ҥH Driver \A4W\A8ӫᰨ\A4W, \BDг]\A9w? 0x40[15:11] -> \A1\A70x00\A1\A8, \B1N\A5\A6Disable
...
u16 CRC16(u8 data,u16 CRC)
{
	...
	for(index=0;index<8;index++)
	{
		...
		CRC=CRC_Result; //\AD\AB\BDƨϥΤW\A4@\A6\B8\AA\BA\B5\B2\AAG
	}
